/*
 * Killocan 2014
 * http://killocan.blogspot.com
*/

#include "spritefiles.h"

namespace mm_spritefiles
{
  const char * sprite_files[] = 
  {
    "./game_data/megaman_seq.dat",
    "./game_data/bullets_seq.dat",
    "dummy",
    "./game_data/blader_seq.dat",
    "./game_data/beak_seq.dat",
    "./game_data/supercutter_seq.dat",
    "./game_data/flyingshell_seq.dat",
    "./game_data/killerbullet_seq.dat",
    "./game_data/spine_seq.dat",
    "./game_data/met_seq.dat",
    "./game_data/picketman_seq.dat",
    "./game_data/sniperjoe_seq.dat",
    "./game_data/watcher_seq.dat",
    "./game_data/tacklefire_seq.dat",
    "./game_data/peng_seq.dat",
    "./game_data/footholder.dat",
    "./game_data/crazyrazy_seq.dat",
    "./game_data/bigeye_seq.dat",
    "./game_data/flea_seq.dat",
    "./game_data/bombomb_seq.dat",
    "./game_data/octopusbattery_seq.dat", //H
    "./game_data/octopusbattery_seq.dat", //V
    "./game_data/screwbomber_seq.dat",
    "./game_data/platform.dat",
    "./game_data/move_platform_seq.dat",
    "./game_data/sparkle_seq.dat",
    "./game_data/lightning_seq.dat",
    "./game_data/firebolt_seq.dat",
    "./game_data/fire_v_seq.dat",
    "./game_data/platform_weapon.dat",
    "./game_data/recharge_all.dat",
    "./game_data/life_charger_big_seq.dat",
    "./game_data/life_charger_little_seq.dat",
    "./game_data/weapon_charger_big_seq.dat",
    "./game_data/weapon_charger_little_seq.dat",
    "./game_data/gutsman_rock_seq.dat",
    "./game_data/new_life_item_seq.dat",
    "./game_data/bullets_seq.dat",
    "./game_data/bullets_seq.dat",
    "./game_data/bullets_seq.dat",
    "./game_data/ice_slasher_seq.dat",
    "./game_data/hyper_bomb_seq.dat",
    "./game_data/bullets_seq.dat",
    "./game_data/bullets_seq.dat",
    "./game_data/bossdoor_seq.dat",
    "./game_data/hit_explosion_seq.dat",
    "./game_data/explosion_little_seq.dat",
    "./game_data/megaman_explode_seq.dat",
    "./game_data/picketman_hammer_seq.dat",
    "./game_data/crazyrazy_lower_seq.dat",
    "./game_data/crazyrazy_upper_seq.dat",
    "./game_data/sparkle_enm_seq.dat",
    "./game_data/bonus_point_seq.dat",
    "./game_data/gutsman_seq.dat",
  };
}
